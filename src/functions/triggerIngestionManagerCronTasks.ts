...
import { IngestionTaskStatus } from "./ingestion/interfaces"; // Import IngestionTaskStatus
...
export default async function (ctx: GSContext): Promise<GSStatus> {
  ...
  logger.info("Attempting to trigger all enabled ingestion tasks via GlobalIngestionLifecycleManager.");
  ...
  const scheduleResult = await globalIngestionManager.scheduleTask({
    id: newManualTaskId,
    name: 'Dynamically Scheduled Manual Task',
    enabled: true,
    trigger: { type: 'manual' },
    source: { pluginType: 'http-crawler', config: { startUrl: 'https://example.com/dynamic' } },
  ...
  // d. Update an existing task (e.g., the 'daily-website-crawl' task)
  const existingTaskId = 'daily-website-crawl'; // ID from test-run.ts
  const updateResult = await globalIngestionManager.updateTask(existingTaskId, {
    description: 'Updated description from cron trigger function',
    enabled: true // Ensure it stays enabled
  });
  logger.info(`[Manager Check] Update task '${existingTaskId}': ${updateResult.message}`, { success: updateResult.success });
  const updatedTask = globalIngestionManager.getTask(existingTaskId);
  logger.info(`[Manager Check] Updated task '${existingTaskId}' status: ${updatedTask?.currentStatus}, desc: ${updatedTask?.description}`);
